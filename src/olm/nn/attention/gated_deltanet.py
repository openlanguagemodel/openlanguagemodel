...
class GatedDeltaNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        alpha = torch.sigmoid(self.alpha_proj(x_conv))  # [B, N, num_key_heads]
        beta = torch.sigmoid(self.beta_proj(x_conv))    # [B, N, num_key_heads]
        ...
        for h in range(self.num_key_heads):
            q_h = q[:, :, h]  # [B, N, key_head_dim]
            k_h = k[:, :, h]  # [B, N, key_head_dim]
            alpha_h = alpha[:, :, h : h + 1]  # [B, N, 1]
            beta_h = beta[:, :, h : h + 1]    # [B, N, 1]
            ...
            for t in range(N):
                k_t = k_h[:, t].unsqueeze(-1)             # [B, key_dim, 1]
                v_t = v_h[:, t].unsqueeze(1)               # [B, 1, v_dim]
                a_t = alpha_h[:, t].unsqueeze(-1)           # [B, 1, 1]
                b_t = beta_h[:, t].unsqueeze(-1)            # [B, 1, 1]

                S = a_t * S + b_t * torch.bmm(k_t, v_t)

                q_t = q_h[:, t].unsqueeze(1)               # [B, 1, key_dim]
                o_t = torch.bmm(q_t, S).squeeze(1)          # [B, v_dim]
                outs.append(o_t)

            output_heads.append(torch.stack(outs, dim=1))  # [B, N, v_dim]

        out = torch.cat(output_heads, dim=-1)  # [B, N, total_v_dim]
