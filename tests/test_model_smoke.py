...
import olm.models.alibaba.qwen3 as qwen3_module
...
from olm.models.meta import Llama2Model, Llama3Model, Llama4MaverickModel
from olm.models.microsoft import Phi3Model, Phi4Model, Phi4_14B
from olm.models.minimax import MiniMaxM2Model
from olm.models.mistralai import MistralLarge3Model, MistralSmall3_1_Model
from olm.models.moonshotai import KimiLinearModel, KimiK2Model
...
def _head_weight(model):
    ...
    if isinstance(
        model,
        (DeepSeekV3Model, KimiK2Model, Gemma3Model, Llama4MaverickModel),
    ):
        # These models keep transformer layers in a separate ModuleList
        # (see forward()), so blocks is [embedding, final_norm, output_head].
        return model.blocks[2].weight
...
def test_deepseek_v3_uses_mla_with_dense_then_moe_layers_and_mtp():
    from olm.nn.attention import MultiHeadLatentAttention
    from olm.nn.feedforward import SwiGLUFFN
    from olm.nn.moe import MoEFeedForward

    model = DeepSeekV3Model(
        128, 32, 4, 4, 16, 8, 4, 4, 4, 8, 16, 16, 4, 1, 2,
        first_k_dense_replace=3, num_mtp_modules=1,
    )
    blocks = model.transformer_blocks
    assert all(isinstance(b.self_attn, MultiHeadLatentAttention) for b in blocks)

    assert all(isinstance(b.ffn, SwiGLUFFN) for b in blocks[:3])
    assert isinstance(blocks[3].ffn, MoEFeedForward)
    assert blocks[3].ffn.num_shared_experts == 1
    assert blocks[3].ffn.router.scoring_func == "sigmoid"
    assert blocks[3].ffn.router.routing_method == "noaux_tc"

    assert model.mtp is not None
    logits, mtp_logits = model.forward_with_mtp(torch.randint(0, 128, (1, 8)))
    assert len(mtp_logits) == 1


def test_deepseek_v3_671b_reference_preset():
    with patch.object(
        deepseek_v3_module.DeepSeekV3Model, "__init__", return_value=None
    ) as init:
        deepseek_v3_module.DeepSeekV3_671B()

    kwargs = init.call_args.kwargs
    assert kwargs["tie_weights"] is False
    assert kwargs["vocab_size"] == 129280
    assert kwargs["embed_dim"] == 7168
    assert kwargs["num_layers"] == 61
    assert kwargs["kv_lora_rank"] == 512
    assert kwargs["q_lora_rank"] == 1536
    ...
    assert kwargs["top_k"] == 8
    assert kwargs["first_k_dense_replace"] == 3
    assert kwargs["num_mtp_modules"] == 1


def test_kimi_k2_uses_mla_with_dense_then_moe_layers():
    from olm.nn.attention import MultiHeadLatentAttention
    from olm.nn.feedforward import SwiGLUFFN
    from olm.nn.moe import MoEFeedForward
    ...
    assert isinstance(blocks[0].ffn, SwiGLUFFN)
    assert all(isinstance(b.ffn, MoEFeedForward) for b in blocks[1:])
    assert all(b.ffn.router.scoring_func == "sigmoid" for b in blocks[1:])
    assert all(b.ffn.router.routing_method == "noaux_tc" for b in blocks[1:])


def test_kimi_k2_1t_reference_preset():
    with patch.object(
        kimi_k2_module.KimiK2Model, "__init__", return_value=None
    ) as init:
        kimi_k2_module.KimiK2_1T()
    ...
    assert kwargs["embed_dim"] == 7168
    assert kwargs["num_layers"] == 61
    assert kwargs["num_heads"] == 64
    assert kwargs["num_experts"] == 384
    assert kwargs["top_k"] == 8
    assert kwargs["routed_scaling_factor"] == 2.827
    assert kwargs["first_k_dense_replace"] == 1


def test_gemma3_alternates_local_and_global_attention_with_distinct_rope_theta():
    from olm.nn.attention import GroupedQueryAttention, SlidingWindowAttention

    model = Gemma3Model(
        128, 32, 64, 8, 4, 2, 8, max_seq_len=16,
        sliding_window=4, sliding_window_pattern=6,
    )
    blocks = model.transformer_blocks
    attn_types = [type(b.self_attn) for b in blocks]

    # 5 local (sliding-window) layers followed by 1 global (full) layer.
    assert attn_types == [
        SlidingWindowAttention, SlidingWindowAttention, SlidingWindowAttention,
        SlidingWindowAttention, SlidingWindowAttention, GroupedQueryAttention,
        SlidingWindowAttention, SlidingWindowAttention,
    ]
    assert blocks[0].self_attn.use_qk_norm
    assert blocks[5].self_attn.use_qk_norm
    assert blocks[0].self_attn.rope.base == 10000.0
    assert blocks[5].self_attn.rope.base == 1000000.0


def test_gemma3_reference_presets_are_untied():
    with patch.object(
        gemma3_module.Gemma3Model, "__init__", return_value=None
    ) as init:
        gemma3_module.Gemma3_4B()
        gemma3_module.Gemma3_27B()

    four_b, twenty_seven_b = [call.kwargs for call in init.call_args_list]
    assert four_b["vocab_size"] == 262144
    assert four_b["embed_dim"] == 2560
    assert four_b["num_layers"] == 34
    assert four_b["sliding_window_pattern"] == 6
    assert twenty_seven_b["embed_dim"] == 5376
    assert twenty_seven_b["num_layers"] == 62
    assert twenty_seven_b["num_kv_heads"] == 16


def test_mistral_small3_1_24b_reference_preset():
    with patch.object(
        mistral_small3_1_module.MistralSmall3_1_Model, "__init__", return_value=None
    ) as init:
        mistral_small3_1_module.MistralSmall3_1_24B()

    kwargs = init.call_args.kwargs
    assert kwargs["tie_weights"] is False
    assert kwargs["vocab_size"] == 131072
    assert kwargs["embed_dim"] == 5120
    assert kwargs["num_layers"] == 40
    assert kwargs["num_heads"] == 32
    assert kwargs["num_kv_heads"] == 8
    assert kwargs["rope_theta"] == 1000000000.0


def test_llama4_maverick_alternates_chunked_nope_attention_and_dense_moe():
    from olm.nn.attention import GroupedQueryAttention, SlidingWindowAttention

    model = Llama4MaverickModel(
        128, 32, 64, 64, 8, 4, 2, 8, max_seq_len=32, chunk_size=8,
        num_experts=4, top_k=1, interleave_moe_layer_step=2, nope_layer_interval=4,
    )
    blocks = model.transformer_blocks
    attn_types = [type(b.self_attn) for b in blocks]
    is_nope = [b.self_attn.rope is None for b in blocks]
    is_moe = [b.use_moe for b in blocks]

    # 3 chunked-local layers then 1 global NoPE layer, repeated.
    assert attn_types == [
        SlidingWindowAttention, SlidingWindowAttention, SlidingWindowAttention,
        GroupedQueryAttention, SlidingWindowAttention, SlidingWindowAttention,
        SlidingWindowAttention, GroupedQueryAttention,
    ]
    assert is_nope == [False, False, False, True, False, False, False, True]
    # Dense/MoE alternates independently of the attention pattern.
    assert is_moe == [False, True, False, True, False, True, False, True]


def test_llama4_maverick_400b_reference_preset():
    with patch.object(
        llama4_module.Llama4MaverickModel, "__init__", return_value=None
    ) as init:
        llama4_module.Llama4Maverick_400B()

    kwargs = init.call_args.kwargs
    assert kwargs["tie_weights"] is False
    assert kwargs["vocab_size"] == 202048
    assert kwargs["embed_dim"] == 5120
    assert kwargs["num_layers"] == 48
    assert kwargs["num_experts"] == 128
    assert kwargs["top_k"] == 1
    assert kwargs["interleave_moe_layer_step"] == 2
    assert kwargs["nope_layer_interval"] == 4


def test_qwen3_uses_qk_norm_and_moe_every_layer():
    from olm.nn.feedforward import SwiGLUMoEFFN

    model = Qwen3Model(
        128, 32, 16, 4, 4, 2, 16, num_experts=4, top_k=2, head_dim=8,
    )
    layers = model.blocks[1].stack

    attn = layers[0].blocks[0].block.blocks[1]
    assert attn.use_qk_norm

    ffns = [layer.blocks[1].block.blocks[1] for layer in layers]
    assert all(isinstance(f, SwiGLUMoEFFN) for f in ffns)
    assert all(f.num_shared_experts == 0 for f in ffns)


def test_qwen3_235b_a22b_reference_preset():
    with patch.object(
        qwen3_module.Qwen3Model, "__init__", return_value=None
    ) as init:
        qwen3_module.Qwen3_235B_A22B()

    kwargs = init.call_args.kwargs
    assert kwargs["tie_weights"] is False
    assert kwargs["vocab_size"] == 151936
    assert kwargs["embed_dim"] == 4096
    assert kwargs["num_layers"] == 94
    assert kwargs["num_heads"] == 64
    assert kwargs["num_kv_heads"] == 4
    assert kwargs["num_experts"] == 128
    assert kwargs["top_k"] == 8
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (qwen3_module, "Qwen3Model", ["Qwen3_235B_A22B"]),
        (deepseek_v3_module, "DeepSeekV3Model", ["DeepSeekV3_671B"]),
        (kimi_k2_module, "KimiK2Model", ["KimiK2_1T"]),
        (gemma3_module, "Gemma3Model", ["Gemma3_4B", "Gemma3_27B"]),
        (
            mistral_small3_1_module,
            "MistralSmall3_1_Model",
            ["MistralSmall3_1_24B"],
        ),
        (llama4_module, "Llama4MaverickModel", ["Llama4Maverick_400B"]),
    ],
)
