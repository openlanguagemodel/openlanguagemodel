"""HF LlamaForCausalLM -> OLM Llama3Model weight map.

Layout notes:
  - q_proj / k_proj rows are permuted from HF's half-split (rotate_half) RoPE
    convention to OLM's interleaved convention; this is an exact
    re-parameterization (see ``permute_rope_rows``).
  - OLM's fused SwiGLU ``up_proj`` chunks its output into ``(value, gate)``,
    so the HF ``up_proj`` (value) rows come first and ``gate_proj`` rows
    second.
  - ``lm_head.weight`` is tied to ``model.embed_tokens.weight`` and is never
    copied separately.
"""
